...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            rangeProofs[i][j].out === 1;

            solRangeProofs[i][j] = RangeProof(32);
            solRangeProofs[i][j].range[0] <== 0;
            solRangeProofs[i][j].range[1] <== 9;
            solRangeProofs[i][j].in <== solution[i][j];
            solRangeProofs[i][j].out === 1;
